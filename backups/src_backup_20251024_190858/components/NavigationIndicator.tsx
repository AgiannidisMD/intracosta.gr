...
const NavigationIndicator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const throttleTime = scrollDirection === 'up' ? 50 : 33; // 20fps for up, 30fps for down
      ...
      // Skip processing if direction hasn't changed and we're scrolling up
      if (scrollDirection === 'up' && lastScrollDirection === 'up' && Math.abs(currentScrollY - lastScrollY) < 10) {
        return;
      }
